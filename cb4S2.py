...
def __zeros__(shape,dtype,max_mem_frac=0.3):
    ...
    arr = np.prod(np.array(shape,dtype=np.int64))*np.zeros(1,dtype=dtype).nbytes * to_gb
...
class S2_MSI_Image(object):
    def __init__(self, S2_MSI_granule_path,
        ...
        band_map = {'B1':'B01', 'B2':'B02', 'B3':'B03', 'B4':'B04', 'B5':'B05', 'B6':'B06', 'B7':'B07', 'B8':'B08',
        ...
        for iband, band in enumerate(self.band_list):
            ...
            print("Read band %s in %.2fs, pure load time:%.2fs, resample time: %.2fs, zoom: %.3f, final shape: %s, index: %i" %
    ...
    def S2_image_to_rgb(self,rgb_bands = ("B11","B08","B03"),rgb_gamma = (1.0,1.0,1.0),pixel_dd_hist = 20,pixel_dd = 1,hist_chop_off = 0.01):
        ...
        for i_rgb,(band,gamma) in enumerate(zip(rgb_bands,rgb_gamma)):
            ...
            S2_rgb[:,:,i_rgb] = rescale_intensity(image=self.data[::pixel_dd,::pixel_dd,i_band],in_range=lim,out_range=(0.0,1.0))
            if gamma != 0.0:
...
class S2_MSI_Classifier(object):
    def __init__(self, cb_clf, mask_legend,clf_to_col,processing_tiles=10):
        ...
        self.S2_MSI_channels = ['B01', 'B02', 'B03', 'B04', 'B05', 'B06', 'B07', 'B08', 'B8A', 'B09', 'B10','B11', 'B12']
...
def mask_image(args, S2_MSI_granule_path):
    ...
    print("Time to read data: %.2f" % (time()-t0,))

    basename_output = path.basename(S2_img.band_fns["B01"]).split("_B01.jp2")[0]

    if args.output_directory == "":
        path_output = path.dirname(S2_img.band_fns["B01"])
    else:
    ...
    t1 = time()
    print("Detection performed in %.2fs -> %.2f Mpx/s" % (t1-t0,S2_img.data.shape[0]*S2_img.data.shape[1]/10**6/(t1-t0)))
...
def main(args):
    ...
    S2_clf = S2_MSI_Classifier(cb_clf=cb_clf, mask_legend=mask_legend,
                               processing_tiles=args.processing_tiles,
                               clf_to_col=clf_to_col)
    exec("builtins.S2_clf = S2_clf")

    if args.S2_MSI_granule_path is None:
        #args.S2_MSI_granule_path = [res for res in glob("**",recursive=True) if re.search('.*GRANULE.*N[0-9]{2}.[0-9]{2}$',res)]
    ...
    if args.number_of_threads == 1:
        ...
        def _init_():
            ...
            sys.stderr = Stdout_to_list()

        print("Start Processing in parallel running threads. Output of in individual jobs is shown when done. ")
        ...
        while len(jobs) > 0:
            ...
            for job, ready in zip(jobs, readies):
                if ready is True:
                    ...
                    for line in lne_out: print(line)
                    if len(lne_err) > 1:
...
class Gui(tk.Tk):
    ...
    def init_gui(self):
        ...
        self.check_rgb_mask = tk.Checkbutton(frame,text="MASK-RGB",onvalue=1,offvalue=0,variable=self.exp_to_rgb_mask)
        ...
        self.check_blend_mask = tk.Checkbutton(frame,text="MASK-BLEND",onvalue=1,offvalue=0,variable=self.exp_to_blend_mask)
        self.check_blend_mask.pack(side=tk.LEFT,fill=tk.X)
        ToolTip(self.check_blend_mask, text=texts["TT_export_to_RGB_mask"])

        self.exp_confidence = tk.IntVar()
        self.exp_confidence.set(1)
        self.check_confidence = tk.Checkbutton(frame,text="confidence",onvalue=1,offvalue=0,variable=self.exp_confidence)
        ...
        tk.Label(frame, text="Tiles:").pack(side=tk.LEFT,fill=tk.X)
        ...
        sys.stdout = Std_redirector(tk_text, gui=self)
        sys.stderr = Std_redirector(tk_text, gui=self)
    ...
    def get_input_file(self):
        ...
        args.S2_MSI_granule_path = []
        for ii, line in enumerate(lines):
            pp = line.strip()
            if path.isdir(pp):
                print("%i:%s" % (ii, pp))
                args.S2_MSI_granule_path.append(pp)
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("-p", "--persistence_file", help="asd", action="store", type=str, default="./cb_data_20151211.pkl") # "./cb_data_20151210.pkl")
